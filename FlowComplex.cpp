...
void flow_complex(Delaunay &delaunay, std::vector<std::array<double, 3> > &vertices, std::vector<std::array<size_t, 3>> &faces, std::map<Delaunay::Vertex_handle, size_t> &vertex_to_index) {
    ...
    for (auto face = delaunay.facets_begin(); face != delaunay.facets_end(); ++face) {
            ...
            if (facet_vertices.size() == delaunay.maximal_dimension() && is_index_two_critical_point(facet_vertices)) {
                ...
                double radius;
                Eigen::VectorXd i(delaunay.maximal_dimension()), j(delaunay.maximal_dimension()), l(delaunay.maximal_dimension());
                i << facet_vertices[0]->point()[0], facet_vertices[0]->point()[1], facet_vertices[0]->point()[2];
                j << facet_vertices[1]->point()[0], facet_vertices[1]->point()[1], facet_vertices[1]->point()[2];
                l << facet_vertices[2]->point()[0], facet_vertices[2]->point()[1], facet_vertices[2]->point()[2];
